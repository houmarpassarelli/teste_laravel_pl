...
<div class="row">
    ...
    <div class="form-group col-12 col-md-4">
        ...
        <input type="text" name="crm" id="crm" class="form-control @error('crm') is-invalid @enderror" value="{{ old('crm', $payload->crm ?? '') }}" autofocus @required(true) />
    </div>
    <div class="form-group col-12 col-md-4">
        ...
        <select class="form-control" name="expertise_id" @required(true)>
            @forelse ($expertises->getExpertises() as $expertise)
                <option value="{{ $expertise->id }}" {{ old('expertise_id', $payload->expertise_id ?? '') == $expertise->id ? 'selected' : '' }}>
                    {{ $expertise->label }}
                </option>
            @empty
